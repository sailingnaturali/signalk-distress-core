...
const AIS_SART = {
  source: 'ais', category: 'distress', deviceBeacon: 'sart', mmsi: '970321098',
  state: 'ais-sart', position: { latitude: 48.795, longitude: -123.265 },
};
const AIS_MOB = { source: 'ais', category: 'distress', deviceBeacon: 'mob', mmsi: '972321098', position: { latitude: 48.795, longitude: -123.265 } };
const AIS_EPIRB = { source: 'ais', category: 'distress', deviceBeacon: 'epirb', mmsi: '974321098', position: { latitude: 48.795, longitude: -123.265 } };

test('AIS SART is spoken as an active SART distress', () => {
  const msg = buildMessage(AIS_SART, { ownPosition: OWN_POSITION });
  assert.match(msg, /^AIS distress: SART active, 2\.3 nautical miles northwest\. Monitor channel 16\.$/);
});

test('AIS MOB and EPIRB have their own spoken forms', () => {
  assert.match(buildMessage(AIS_MOB, { ownPosition: OWN_POSITION }), /^AIS man-overboard beacon, /);
  assert.match(buildMessage(AIS_EPIRB, { ownPosition: OWN_POSITION }), /^AIS EPIRB beacon, /);
});

test('AIS logbook entry records device type, MMSI and position', () => {
  const text = buildLogbookText(AIS_EPIRB, {});
  assert.match(text, /^\[AIS\] EPIRB beacon/);
