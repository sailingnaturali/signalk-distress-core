...
test('bucketOf/label/describe/colors are injectable for non-DSC sources', () => {
  const { buildMarkerResourceSets } = require('../lib/markers');
  const now = Date.parse('2026-06-30T20:19:30.000Z');
  const events = [{
    category: 'distress', deviceBeacon: 'epirb', mmsi: '974321098',
    position: { latitude: 48.79, longitude: -123.26 },
    receivedAt: '2026-06-30T20:19:30.000Z',
  }];
  const sets = buildMarkerResourceSets(events, {
    now,
    bucketOf: (e) => e.deviceBeacon,
    label: (b) => `AIS — ${b}`,
    describe: (b) => `AIS ${b} distress beacons`,
    colors: { epirb: 'rgba(211,47,47,1)' },
  });
  assert.equal(sets.epirb.type, 'ResourceSet');
  assert.equal(sets.epirb.name, 'AIS — epirb');
  assert.equal(sets.epirb.description, 'AIS epirb distress beacons');
