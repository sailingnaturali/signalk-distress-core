...
function buildMessage(event, { ownPosition, vesselName } = {}) {
  ...
  if (event.source === 'ais' && event.kind === 'safetyBroadcast') {
    const lead = { distress: 'AIS MAYDAY relay', urgency: 'AIS urgency broadcast', safety: 'AIS safety broadcast' }[event.category] || 'AIS broadcast';
    return `${lead}: ${event.text}. Monitor channel 16.`;
  }
  if (event.source === 'ais') {
...
function buildLogbookText(event, { ownPosition, vesselName } = {}) {
  ...
  if (event.source === 'ais' && event.kind === 'safetyBroadcast') {
    const kind = { distress: 'MAYDAY RELAY', urgency: 'URGENCY', safety: 'SAFETY' }[event.category] || 'BROADCAST';
    parts.push(`AIS ${kind} broadcast from ${name}: ${event.text}`);
  } else if (event.source === 'ais') {
