...
class EventStore {
  ...
  /** Newest event matching `predicate` last heard within `windowMs` of `nowMs`.
   *  Recency is measured from `lastReceivedAt` when a repeat has bumped it, so a
   *  beacon that keeps transmitting slides its own window forward instead of a
   *  fresh event being minted every window. `lastReceivedAt` breaks the
   *  receivedAt ordering the old early-exit relied on, so we scan the whole tail
   *  — fine given how rare this traffic is. */
  findRecent(predicate, nowMs, windowMs) {
    for (let i = this.events.length - 1; i >= 0; i--) {
      const e = this.events[i];
      const age = nowMs - Date.parse(e.lastReceivedAt || e.receivedAt);
      if (age >= 0 && age <= windowMs && predicate(e)) return e;
    }
    return undefined;
  }
}
